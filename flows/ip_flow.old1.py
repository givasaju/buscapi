...
class PropriedadeIntelectualFlow(Flow[PropriedadeIntelectualState]):
    ...
    def __init__(self):  # ✅ CORREÇÃO: Remover argumentos do __init__
        """
        IMPORTANTE: CrewAI Flow.__init__() não aceita argumentos.
        Configuração deve ser feita após a inicialização.
        """
        super().__init__()
        ...
        # Conexão com banco será inicializada quando necessário
        self.db = None
...
class IPFlowManager:
    ...
    def create_flow(self, flow_id: str, search_criteria: str) -> PropriedadeIntelectualFlow:
        """
        Cria um novo fluxo.
        ✅ CORREÇÃO: Criar fluxo sem argumentos e configurar depois.
        """
        # Criar fluxo sem argumentos
        flow = PropriedadeIntelectualFlow()
        
        # Configurar critérios após criação
        flow.setup_flow(search_criteria)
        
        # Armazenar fluxo ativo
    ...
    def get_flow_status(self, flow_id: str) -> Dict[str, Any]:
        """Retorna status de um fluxo específico."""
        flow = self.get_flow(flow_id)
        if not flow:
            return {"error": "Fluxo não encontrado"}
        ...
        return {
            ...
            "current_step": flow.state.flow_id,
